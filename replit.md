...
- **Synchronization (Book 13):** offline-first sync for GovCore Nodes — an append-only per-tenant change log with monotonic sequence numbers, cursor-based pull batching, push reconciliation, and **per-entity conflict policies** (manual | last_write_wins | server_wins | node_wins) registered as data. `@workspace/sync-engine`, package at `lib/sync-engine/`
...
- **Superadmin status requires a reserved role code AND `isSystem`.** Superadmin now means genuine cross-tenant access, and role codes are tenant-scoped and API-creatable — so `routes/roles.ts` refuses reserved codes and never accepts `isSystem` from a request body, and `isSuperadminGrant` requires both. The only `isSystem` superadmin role is written by the bootstrap path in `ensureGovCoreUser`. Both halves are load-bearing.
- **The module permission check fails closed.** A route family mapped in `moduleForPath` with no permission rows in the database is reachable by nobody but a superadmin. `reports`, `sync`, and `treasury` were mapped after their routes shipped, so existing databases need `pnpm --filter @workspace/scripts run seed:permissions` and then explicit grants.
- **Treasury hooks are hand-authored and live OUTSIDE `lib/api-client-react/src/generated/`.** Orval's `clean` wipes that folder on every codegen run; anything hand-written inside it disappears. Treasury is also absent from `openapi.yaml`, which is why it needs the local fetch adapter in `src/treasury.ts`.
- **Sync conflict policy is per entity type, and defaults to `manual`.** Registering an entity for sync without naming a policy means divergences escalate to a human rather than auto-resolving — the safe default for government records. `last_write_wins` trusts node wall-clocks; ties and missing timestamps deterministically favour the server.
- **A node's sync cursor only moves forward, and only to a seq the server issued.** `isValidCursorAdvance` enforces this; an empty pull batch must leave the cursor where it is. Advancing optimistically would skip changes permanently, since pulls only look forward.
- **Sync seq assignment reads `max(seq)` then inserts** inside a transaction. Under concurrent writes for one tenant this can collide; a production deployment should add a per-tenant sequence or a unique `(tenant_id, seq)` constraint with retry. Not verifiable here — no Postgres in this environment.
